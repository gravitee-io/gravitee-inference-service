...
public class LlamaCppProvider implements InferenceHandlerProvider {
  ...
  public LlamaCppProvider(Vertx vertx, String modelPath) {
    ...
    this.handlerFactory = new LlamaCppInferenceHandlerFactory(
      vertx,
      new LlamaCppModelFactory()
    );
  }
    ...
    if (type != InferenceType.TEXT_GENERATION) {
      return Single.error(
        new IllegalArgumentException(
          "Unsupported inference type '" + type + "' for format LLAMA_CPP"
        )
      );
    }
  ...
  private Single<Path> resolveModelPath(ConfigWrapper config) {
    ...
    if (modelPath == null || modelPath.isBlank()) {
      return Single.error(
        new IllegalArgumentException("modelPath is required")
      );
    }
  ...
  private ModelConfig buildModelConfig(
    ...
    SplitMode splitMode = enumValue(
      SplitMode.class,
      modelParams.get(Constants.MODEL_SPLIT_MODE),
      SplitMode.NONE
    );
    ...
    LlamaLogLevel logLevel = enumValue(
      LlamaLogLevel.class,
      modelParams.get(Constants.MODEL_LOG_LEVEL),
      null
    );
  ...
  private Single<java.util.Optional<Path>> resolveLoraPath(
    ConfigWrapper config
  ) {
    Map<String, Object> modelParams = config.get(
      Constants.MODEL_PARAMS,
  ...
  private Single<Path> fetchRemoteFile(
    String repo,
    String file,
    String modelName
  ) {
  ...
  private LoraSpec parseHuggingFaceUrl(String value) {
    try {
      ...
      String repo = String.join(
        "/",
        java.util.Arrays.copyOfRange(parts, 1, resolveIndex)
      );
      String file = String.join(
        "/",
        java.util.Arrays.copyOfRange(parts, resolveIndex + 2, parts.length)
      );
