...
public class InferenceHandlerTest {
  ...
  public void setUp() {
    ...
    inferenceHandler = new InferenceHandler("test-address", model, vertx);
  }
  ...
  public void must_handle_unsupported_inference_request() {
    InferenceRequest request = new InferenceRequest(CREATE, Map.of(Constants.INPUT, "some-input"));
    when(message.body()).thenReturn(Json.encodeToBuffer(request));

    inferenceHandler.handle(message);

    verify(message).fail(405, "Unsupported action: CREATE");
  }
